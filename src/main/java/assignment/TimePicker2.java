...
public class TimePicker2 extends TextField {
    ...
    private final ReadOnlyStringWrapper am_pm;
    ...
    public TimePicker2() {
        ...
        this.addEventFilter(KeyEvent.KEY_RELEASED, inputevent -> {
            ...
            if (this.getText() != null && this.getText().length() >= 5
                    && this.getText().indexOf(":") == 2) {
                withMinutes = true;
            }
            boolean withSeconds = false;
            if (this.getText() != null && this.getText().length() == 8
                    && this.getText().lastIndexOf(":") == 5) {
                withSeconds = true;
            }
    ...
    public TimePicker2(LocalTime time) {
        super(getClockTime(time));
        timePattern = Pattern.compile("([01]?[0-9]|2[0-3]):[0-5][0-9] [AP]M");
        if (!validate(this.getText())) {
            throw new IllegalArgumentException("Invalid time: " + this.getText());
        }
        hours = new ReadOnlyIntegerWrapper(this, "hours");
        minutes = new ReadOnlyIntegerWrapper(this, "minutes");
        am_pm = new ReadOnlyStringWrapper(this, "am_pm");

        hours.bind(new TimePicker2.TimeUnitBinding(Unit.HOURS));
        minutes.bind(new TimePicker2.TimeUnitBinding(Unit.MINUTES));
        am_pm.bind(new StringBinding() {
            @Override
    ...
    private static String getClockTime(LocalTime time) {
        int hours = time.getHour() >= 12 ? time.getHour() - 12 : time.getHour();
        int minute = time.getMinute();
        String am_pm = time.getHour() < 12 ? "AM" : "PM";
        return String.format("%02d:%02d %s", hours, minute, am_pm);
    }
    ...
    private boolean validate(String time) {
        ...
        try {
            ...
            if (!tokens[2].matches("[AP]M")) return false;
            return true;
